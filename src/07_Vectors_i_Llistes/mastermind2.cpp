...
Comb primer_candidat(list<Comb>& L) {
  list<Comb>::iterator i = find_if(L.begin(), L.end(), 
				   numeros_diferents);
...
int main() {
  list<Comb> targets;
  totes_les_combinacions(targets);
  list<Comb>::iterator i = targets.begin(), iend = targets.end();
  ...
  for (; i != iend; i++) {
    ...
    while (!candidats.empty()) {
      ...
      pair<int, int> _mf = prova.morts_i_ferits(target);
      int morts = _mf.first, ferits = _mf.second;    
      
      list<Comb>::iterator j = candidats.begin();
      while (j != candidats.end()) {
	pair<int, int> m_i_f = prova.morts_i_ferits(*j);
	// cerr << prova << ' ' << *j << ' ' << m_i_f.first << ' ' << m_i_f.second << endl;
    ...
    cout << target << ' ' << prova << ' ' << iteracions << endl;
  }
}
